...
These endpoints require an API key with the `admin:uploads` scope. They are only available on `write` or `full` role nodes.
...
Returns all active (non-revoked) API keys. Secrets are never returned.

**Response:**
```json
{
  "keys": [
    { "id": "abc123...", "owner": "0x...", "scopes": ["*"], "tier": "authenticated" }
  ],
  "count": 1
}
```
...
{ "owner": "0x...", "scopes": ["*"], "tier": "authenticated" }
```

Returns the plaintext secret once. Store it securely — it cannot be retrieved later.

### Revoke API Key

```
DELETE /ops/api-keys/:keyId
```

Permanently revokes a key. The key cannot be un-revoked.

### Rotate API Key

```
POST /ops/api-keys/:keyId/rotate
```

Revokes the old key and creates a new one. Returns the new plaintext secret once.
